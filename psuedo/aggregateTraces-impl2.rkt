...
; /*-------------------------------------------------------------------------*/
; /*--- HerbGrind: a valgrind tool for Herbie aggregateTraces-impl2.rkt.c ---*/
; /*-------------------------------------------------------------------------*/
; 
; /*
  ...
  (let* ([tea-map-groups (get-groups (tea-branch-node-map tea))]
    ...
    (for ([positions tea-map-groups] [group-idx (in-naturals)]
      ...
      (let ([split-map : (HashTable Integer Integer)
                       (make-hash)]
            [initial-group : Integer
             (hash-ref stem-map (first positions))])
        (hash-set! split-map initial-group group-idx)
