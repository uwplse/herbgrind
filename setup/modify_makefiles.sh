...
awk -i inplace '/^TOOL_DEPENDENCIES_@VGCONF_PLATFORM_PRI_CAPS@ =/{print;print "\t$(extra_deps_@VGCONF_ARCH_PRI@) \\";next}1' ../valgrind/Makefile.tool.am
awk -i inplace '/^TOOL_DEPENDENCIES_@VGCONF_PLATFORM_SEC_CAPS@ =/{print;print "\t$(extra_deps_@VGCONF_ARCH_SEC@) \\";next}1' ../valgrind/Makefile.tool.am
